...
- **2026-06-08（Phase 4f/4g 完成 — 引擎切换 + 删官方 openai 聊天，本 PR）** — 用户拍板：本仓是 API 客户端层、非带线上流量的服务，**不需要运行时灰度开关、不保留旧实现**，直接做完整切换。
  - **4f 切默认引擎**：两个路由切换点的 default 由官方 `OpenAIProvider` 直接切到 `OpenAIAISDKProvider`：① `services/messages/ApiProvider.ts` 主路径 `createProviderInstance` default；② `services/ai/ProviderFactory.ts` 次路径 `getProviderApi` 的 `openai/deepseek/google/grok/siliconflow/volcengine/azure-openai/default` 分支（抽 `createOpenAICompatibleApi()` 统一构造，`openai-aisdk` case 一并复用去重）。
  - **隐藏耦合迁移**：`DashScopeProvider`（`dashscope/provider.ts`）此前内部委托官方 `OpenAIProvider` 处理聊天，连同 `dashscope/index.ts` 的 `sendChatRequest`（曾 import `openai/chat`）一并切到 `OpenAIAISDKProvider`，否则删官方实现会断 DashScope。
  - **4g 删官方 openai 聊天**：删 `api/openai/{provider,chat,unifiedStreamProcessor}.ts` 及其 `__tests__/stream.characterization.test.ts`（钉的是被删的 `UnifiedStreamProcessor`）；重写 `openai/index.ts`、`openai.ts` redirect 去掉 chat/provider/stream 导出与 `sendChatRequest`/`createOpenAIAPI`。**保留** `image/video/models/multimodal/client/tools.ts`（`client.ts` 被 image/models 依赖、`tools.ts` 的 `WEB_SEARCH_TOOL` 被 openai-aisdk 复用，归 Phase 6）。
  - 三连全绿：type-check / test(41，少了被删 stream 特征测试的 5 例) / build。`tools.characterization.test.ts`（测 `openai/tools.ts` 纯函数）保留有效。
  - **依赖决策落地**：沿用选项 1（`@ai-sdk/openai` 的 `.chat()`），未装 `@ai-sdk/openai-compatible`、未建新目录；`providerOptions.openai` 私有字段透传的 live 实测在 PR 后用 DeepSeek key 在真实 app 内回归（reasoning_content → THINKING_DELTA）。
