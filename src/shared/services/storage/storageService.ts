...
export async function initStorageService(): Promise<void> {
  try {
    ...
    if (cleanupResult.found > 0) {
      logger.debug(`清理了 ${cleanupResult.cleaned}/${cleanupResult.found} 个旧数据库`);
    }

    // Dexie会自动打开数据库，无需手动initialize
    // logger.debug('存储服务: 数据库初始化检查完成，或已通过initialize()处理。');
    ...
    const errorDetails = error instanceof Error && error.stack 
      ? `\n错误堆栈: ${error.stack}` 
      : '';
    logger.error(`存储服务初始化失败: ${errorMessage}${errorDetails}`);
    
    try {
      const isOpen = dexieStorage.isOpen();
      logger.error(`数据库状态: ${isOpen ? '已打开' : '未打开'}`);
      
      if (!isOpen) {
        logger.debug('尝试重新打开数据库...');
        await dexieStorage.open();
        logger.debug('数据库已重新打开');
      }
    } catch (dbError) {
      logger.error('获取数据库状态失败:', 
...
export async function cleanupOldDatabases(): Promise<{
  ...
  try {
    ...
    logger.debug(`清理旧数据库: 找到 ${oldDatabases.length} 个旧数据库`, oldDatabases);

    // 删除旧数据库
    let cleanedCount = 0;
    for (const dbName of oldDatabases) {
      try {
        logger.debug(`正在删除数据库: ${dbName}`);
        await Dexie.delete(dbName);
        cleanedCount++;
        logger.debug(`已删除数据库: ${dbName}`);
      } catch (e) {
        logger.error(`删除数据库 ${dbName} 失败:`, e);
      }
    }
...
export async function getDatabaseStatus(): Promise<DatabaseStatus> { // 使用新的类型
  logger.debug('storageService: 获取数据库状态...');
  try {
    const databases = await Dexie.getDatabaseNames();
    ...
    if (dexieStorage.isOpen()) {
      ...
      try { status.topicsCount = await dexieStorage.topics.count(); } catch(e) { logger.warn('Failed to count topics', e); }
      try { status.assistantsCount = await dexieStorage.assistants.count(); } catch(e) { logger.warn('Failed to count assistants', e); }
      try { status.imagesCount = await dexieStorage.images.count(); } catch(e) { logger.warn('Failed to count images', e); }
      try { status.settingsCount = await dexieStorage.settings.count(); } catch(e) { logger.warn('Failed to count settings', e); }
      try { status.metadataCount = await dexieStorage.metadata.count(); } catch(e) { logger.warn('Failed to count metadata', e); }
      try { status.memoriesCount = await dexieStorage.memories.count(); } catch(e) { logger.warn('Failed to count memories', e); }
      ...
      logger.warn('storageService: 数据库未打开，无法获取详细状态。');
...
class StorageService {
  ...
  setUseDexiePriority(enable: boolean): void {
    logger.debug(`StorageService: ${enable ? '启用' : '禁用'}Dexie存储`);
  }
  ...
  async getAssistant(id: string): Promise<Assistant | null> {
    try {
      ...
      logger.error('StorageService: 获取助手失败', error);
      return null;
    }
  }
  ...
  async getAllAssistants(): Promise<Assistant[]> {
    try {
      ...
      logger.error('StorageService: 获取所有助手失败', error);
      return [];
    }
  }
  ...
  async getTopic(id: string): Promise<ChatTopic | null> {
    try {
      ...
      logger.error('StorageService: 获取话题失败', error);
      return null;
    }
  }
  ...
  async getAllTopics(): Promise<ChatTopic[]> {
    try {
      ...
      logger.error('StorageService: 获取所有话题失败', error);
      return [];
    }
  }
  ...
  async getSetting(key: string): Promise<any> {
    try {
      ...
      logger.error('StorageService: 获取设置失败', error);
      return null;
    }
  }
...
export class CapacitorStorageService {
  ...
  static async setItem(key: string, value: any): Promise<void> {
    try {
      ...
      logger.error('CapacitorStorageService.setItem 失败:', error);
      throw error;
    }
  }
  ...
  static async getItem<T>(key: string, defaultValue: T | null = null): Promise<T | null> {
    try {
      ...
      logger.error('CapacitorStorageService.getItem 失败:', error);
      return defaultValue;
    }
  }
  ...
  static async removeItem(key: string): Promise<void> {
    try {
      ...
      logger.error('CapacitorStorageService.removeItem 失败:', error);
      throw error;
    }
  }
  ...
  static async clear(): Promise<void> {
    try {
      ...
      logger.error('CapacitorStorageService.clear 失败:', error);
      throw error;
    }
  }
  ...
  static async keys(): Promise<string[]> {
    try {
      ...
      logger.error('CapacitorStorageService.keys 失败:', error);
      return [];
    }
  }
  ...
  static async setItems(items: Record<string, any>): Promise<void> {
    try {
      ...
      logger.error('CapacitorStorageService.setItems 失败:', error);
      throw error;
    }
  }
}