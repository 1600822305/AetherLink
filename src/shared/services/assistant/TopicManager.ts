...
export class TopicManager {
  ...
  static async addTopicToAssistant(assistantId: string, topicId: string): Promise<boolean> {
    try {
      logger.debug(`尝试将话题 ${topicId} 添加到助手 ${assistantId}`);

      // 参数验证
      if (!assistantId || !topicId) {
        logger.error('添加话题到助手失败: 无效的参数', { assistantId, topicId });
        return false;
      }
      ...
      if (!assistant) {
        logger.error(`助手 ${assistantId} 不存在`);
        return false;
      }
      ...
      if (!topic) {
        logger.error(`话题 ${topicId} 不存在，无法添加到助手`);
        return false;
      }

      // 验证话题有效性
      if (!TopicStatsService.isValidTopic(topic)) {
        logger.error(`话题 ${topicId} 无效，尝试修复`);

        // 尝试修复话题 - 添加系统提示词
        if (!topic.prompt) {
          topic.prompt = DEFAULT_TOPIC_PROMPT;
          await dexieStorage.saveTopic(topic);
          logger.debug(`已为话题 ${topicId} 添加系统提示词`);
        }

        // 再次验证
        if (!TopicStatsService.isValidTopic(topic)) {
          logger.error(`修复后话题 ${topicId} 仍然无效，无法添加到助手`);
          return false;
        }
      }
      ...
      if (topicIds.includes(topicId)) {
        logger.debug(`助手 ${assistantId} 已包含话题 ${topicId}`);
        return true;
      }
      ...
      if (success) {
        logger.debug(`成功将话题 ${topicId} 添加到助手 ${assistantId}`);
        ...
        try {
          ...
          logger.debug(`已派发topicCreated事件，通知UI更新`);
        } catch (eventError) {
          logger.warn(`派发话题创建事件失败:`, eventError);
        }
      } else {
        logger.error(`无法将话题 ${topicId} 添加到助手 ${assistantId}`);
      }

      return success;
    } catch (error) {
      logger.error(`添加话题到助手失败: ${error}`);
      return false;
    }
  }
  ...
  static async removeTopicFromAssistant(assistantId: string, topicId: string): Promise<boolean> {
    try {
      logger.debug(`尝试从助手 ${assistantId} 移除话题 ${topicId}`);
      ...
      // 检查是否包含该话题
      if (!topicIds.includes(topicId)) {
        logger.debug(`助手 ${assistantId} 不包含话题 ${topicId}`);
        return true;
      }
      ...
      if (success) {
        logger.debug(`成功从助手 ${assistantId} 移除话题 ${topicId}`);
        ...
        logger.error(`无法从助手 ${assistantId} 移除话题 ${topicId}`);
      }

      return success;
    } catch (error) {
      logger.error(`从助手移除话题失败: ${error}`);
      return false;
    }
  }
  ...
  static async clearAssistantTopics(assistantId: string): Promise<boolean> {
    try {
      logger.debug(`开始清空助手 ${assistantId} 的所有话题`);
      ...
      logger.debug(`助手 ${assistant.name} 有 ${originalTopicIds.length} 个话题需要删除:`, originalTopicIds);

      if (originalTopicIds.length === 0) {
        logger.debug(`助手 ${assistant.name} 没有话题，无需清空`);
        return true;
      }
      ...
      const success = await dexieStorage.transaction('rw', [
          ...
          try {
            ...
            if (topic) {
              ...
              if (topic.messageIds && topic.messageIds.length > 0) {
                logger.debug(`删除话题 ${topicId} 的 ${topic.messageIds.length} 条消息`);
              ...
              if (topic.messages && topic.messages.length > 0) {
                logger.debug(`话题 ${topicId} 有 ${topic.messages.length} 条内嵌消息`);
            ...
            logger.debug(`已删除话题 ${topicId}`);
          } catch (error) {
            logger.error(`删除话题 ${topicId} 时出错:`, error);
            // 继续删除其他话题，不中断整个过程
          }
        }
        ...
        logger.debug(`已更新助手 ${assistant.name}，清空话题引用`);

        return true;
      });

      if (success) {
        logger.debug(`成功清空助手 ${assistantId} 的所有话题，共删除 ${originalTopicIds.length} 个话题`);
        ...
        window.dispatchEvent(event);
        logger.debug('已派发topicsCleared事件', { assistantId, originalTopicIds });

        return true;
      } else {
        logger.error(`清空助手 ${assistantId} 的话题失败`);
        return false;
      }
    } catch (error) {
      logger.error(`清空助手话题失败:`, error);
      return false;
    }
  }
  ...
  static async ensureAssistantHasTopic(assistantId: string): Promise<ChatTopic> {
    try {
      logger.debug(`检查助手 ${assistantId} 的话题`);

      // 获取助手 - 使用dexieStorage替代dataService
      const assistant = await dexieStorage.getAssistant(assistantId);
      if (!assistant) {
        logger.error(`助手 ${assistantId} 不存在，无法获取话题`);
        throw new Error(`助手 ${assistantId} 不存在`);
      }

      logger.debug(`成功获取助手: ${assistant.name} (${assistant.id})`);

      // 检查是否有话题
      const topicIds = Array.isArray(assistant.topicIds) ? assistant.topicIds : [];
      logger.debug(`助手 ${assistant.name} 有 ${topicIds.length} 个话题ID: ${JSON.stringify(topicIds)}`);

      if (topicIds.length > 0) {
        // 获取第一个话题
        const firstTopicId = topicIds[0];
        logger.debug(`尝试获取第一个话题: ${firstTopicId}`);

        try {
          const topic = await TopicService.getTopicById(firstTopicId);

          if (topic) {
            logger.debug(`助手 ${assistant.name} 已有有效话题: ${topic.title || topic.name} (${topic.id})`);
            return topic;
          } else {
            logger.warn(`话题ID ${firstTopicId} 存在于助手的topicIds中，但无法获取话题数据`);
          }
        } catch (topicError) {
          logger.error(`获取话题 ${firstTopicId} 时出错:`, topicError);
        }
      }

      // 助手没有有效话题，抛出错误
      logger.error(`助手 ${assistant.name} 没有有效话题`);
      throw new Error(`助手 ${assistant.name} 没有有效话题，请使用 AssistantFactory.createAssistant 创建助手时自动创建默认话题`);
    } catch (error) {
      logger.error(`获取助手话题失败:`, error);
      throw error;
    }
  }
  ...
  static async validateAndFixAssistantTopicReferences(assistantId: string): Promise<{
    ...
    try {
      logger.debug(`验证助手 ${assistantId} 的话题引用`);

      // 获取助手 - 使用dexieStorage替代dataService
      const assistant = await dexieStorage.getAssistant(assistantId);
      if (!assistant) {
        logger.error(`助手 ${assistantId} 不存在，无法验证话题引用`);
      ...
      const topicIds = Array.isArray(assistant.topicIds) ? assistant.topicIds : [];
      if (topicIds.length === 0) {
        logger.debug(`助手 ${assistant.name} 没有关联话题，无需修复`);
        return { fixed: false, removedCount: 0, validCount: 0 };
      }

      logger.debug(`助手 ${assistant.name} 有 ${topicIds.length} 个话题引用，开始验证`);
      ...
      for (const topicId of topicIds) {
        try {
          ...
          if (topic && TopicStatsService.isValidTopic(topic)) {
            ...
            logger.debug(`话题 ${topicId} 无效或不存在，将从助手中移除`);
            invalidTopicIds.push(topicId);
          }
        } catch (error) {
          logger.error(`验证话题 ${topicId} 时出错:`, error);
          invalidTopicIds.push(topicId);
        }
      }
      ...
      if (invalidTopicIds.length === 0) {
        logger.debug(`助手 ${assistant.name} 的所有话题引用都有效，无需修复`);
      ...
      if (success) {
        logger.debug(`成功修复助手 ${assistant.name} 的话题引用，移除了 ${invalidTopicIds.length} 个无效引用`);
        return { fixed: true, removedCount: invalidTopicIds.length, validCount: validTopicIds.length };
      } else {
        logger.error(`无法更新助手 ${assistant.name} 的话题引用`);
        return { fixed: false, removedCount: 0, validCount: validTopicIds.length };
      }
    } catch (error) {
      logger.error(`验证并修复助手话题引用时出错:`, error);
      return { fixed: false, removedCount: 0, validCount: 0 };
    }
  }
  ...
  static async validateAndFixAllAssistantsTopicReferences(): Promise<{
    ...
    try {
      ...
      logger.debug(`验证完成，修复了 ${assistantsFixed} 个助手的话题引用，共移除 ${totalRemoved} 个无效引用`);
      return { assistantsFixed, totalAssistants: assistants.length, totalRemoved };
    } catch (error) {
      logger.error('验证所有助手话题引用时出错:', error);
      return { assistantsFixed: 0, totalAssistants: 0, totalRemoved: 0 };
    }
  }
  ...
  static async addAssistantMessagesToTopic({ assistant, topic }: { assistant: Assistant; topic: ChatTopic }): Promise<void> {
    try {
      logger.debug(`尝试向话题添加助手初始消息: ${topic.id}`);

      // 确保话题的assistantId正确
      if (topic.assistantId !== assistant.id) {
        logger.warn(`话题 ${topic.id} 的assistantId (${topic.assistantId}) 与助手ID (${assistant.id}) 不匹配，正在修正`);
      ...
      if (!topicIds.includes(topic.id)) {
        logger.debug(`助手 ${assistant.id} 的topicIds不包含话题 ${topic.id}，正在添加`);
      ...
      if (updatedTopic.messageIds && updatedTopic.messageIds.length > 0) {
        logger.debug(`话题 ${updatedTopic.id} 已有消息，不添加初始消息，但确保关联关系正确`);
      } else {
        logger.debug(`话题 ${updatedTopic.id} 没有消息，不添加初始欢迎消息，由用户首次发送消息触发助手响应`);
        // 确保messageIds数组存在
      ...
      logger.debug(`已保存话题 ${updatedTopic.id} 到数据库`);
      ...
      if (assistantNeedsUpdate) {
        ...
        const success = await AssistantManager.updateAssistant(updatedAssistant);
        if (success) {
          logger.debug(`已更新助手 ${assistant.id} 的topicIds`);
        } else {
          logger.error(`更新助手 ${assistant.id} 的topicIds失败`);
        }
      }
      ...
      logger.debug(`成功向话题 ${updatedTopic.id} 添加助手初始消息并确保关联关系正确`);

    } catch (error) {
      logger.error(`添加助手消息到话题失败:`, error);
    }
  }
}